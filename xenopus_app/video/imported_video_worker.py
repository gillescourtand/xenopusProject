...
Imported-video analysis without using the camera acquisition or the legacy
video player.

The maintenance rule is important: this module does not duplicate tracking.
It reads a video frame by frame, creates FramePacket objects, then calls
AppController.tracking_adapter(packet), which is the same tracking path used
by live acquisition.
"""
...
class VideoFileAnalysisWorker(threading.Thread):
    """
    Analyze an imported video in a background thread.

    Results are written to CSV through ResultRecorder and also kept in memory
    by frame id so the UI can review overlays after the analysis.
    """

    def __init__(self, controller, video_path, result_file_path, metadata=None, video_crop=None):
        """Initialize the worker and its status state."""
    ...
    def run(self):
        """Read the video, track each frame, and record all results."""
        cap = None
        recorder = None
    ...
    def _apply_video_crop(self, image):
        """
        Apply the crop selected in the UI.

        ROIs and tail arcs are placed on the cropped image shown in the UI,
        so the analyzed image must use the same coordinate system.
        """
        if image is None:
            return image
    ...
    def stop(self):
        """Request analysis and recording shutdown."""
        self.stop_event.set()
        self.recorder_stop_event.set()
    ...
    def get_status(self):
        """Return a thread-safe copy of the current analysis status."""
        with self._lock:
            return dict(self._status)

    def get_result(self, frame_id):
        """Return the TrackingResult stored for a given frame id."""
        try:
