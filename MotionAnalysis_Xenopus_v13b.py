...
v4b : optimisation du traitement de l'image pour l'affichage - acceleration de la lecture
v5 : implementation de l'analyse live par buffer
v5b : fonction convert placé dans une variable local pouralléger le traitement
...
#modules perso
import pylon_parametersListmodule2c2 as baslerParameters
...
class FileVideoStreamLiveCV2:
    def __init__(self, path, queueSize=1024):
        ...
        #setVideoProperties(video.capture)
        video.LUT=gamma_LUT(1.8) 
        #récupération des propriétés de la vidéo
        #getVideoProperties(video.capture)
        self.stopped = False
...
def getVideoProperties(video_device):
    #fps = cap.get(cv2.cv.CV_CAP_PROP_FPS)=cap.get(5)
    #initialisation du format video : niveaux de gris
    video.pixFormat= video_device.properties['PixelFormat']
    video.fps=video_device.properties['ResultingFrameRate']
    video.width=video_device.properties['Width']
    video.height=video_device.properties['Height']   
...
def setVideoProperties(video_device):
    ...
    #video_device.properties['AcquisitionFrameRateEnable']=True
...
def open_video():
    ...
    filePath,_=QtWidgets.QFileDialog.getOpenFileName(
                    ...
                    "Video file (*.avi;*.mpg;*.mp4;*.mov;*.flv)" # opencv ne prend en charge que le format avi
                )
    print(filePath)
...
def set_cam_parameters(cam):
    print("set_cam_parameters")
    if cam.IsOpen():
        if video.isAcquiring==False :
            ...
            if cam.AcquisitionFrameRateEnable.GetValue()==True :
...
def updatePlot() :
    ...
    if ui.track_checkBox.isChecked()==True:
        ...
        if ui.manipType_comboBox.currentText()=="Pupil":
            dataArray1 = np.asarray(roisEllipseEye[0].diameterList[currentIdx-400: currentIdx])
            #print(dataArray1)
            ...
            if len(ui.tailEllipse.angleList)!=0:
                dataArray3 = np.asarray(ui.tailEllipse.angleList[currentIdx-400: currentIdx])
    ...
    if ui.track_checkBox.isChecked()==False:
        if len(roisEllipseEye)>0 :
            if ui.manipType_comboBox.currentText()=="Pupil":
                ...
                dataArray1 = np.asarray(roisEllipseEye[0].diameterList)
                #print(dataArray[-1])
                #pen=pg.mkPen((0,255,255), width=2)
                ui.w3.plot(dataArray1,pen=ui.penCyan,clear=True) #clear=True permet de remplacer le contenu du plot par un nouveau contenu
                #autopan dans pyqtgraph recentre le graph, il n'est pas possible de voir défiler les dernières valeurs en temps réel
                ...
                """
                dataArray1 = roisEllipseEye[0].diameterList
                #print(dataArray[-1])
                #pen=pg.mkPen((0,255,255), width=2)
                curve1=ui.w3.plot(dataArray1,pen=ui.penCyan,clear=True)
                """
            else :    
                ...
                if len(ui.tailEllipse.angleList)!=0:
                    dataArray3 = np.asarray(ui.tailEllipse.angleList)
                    #pen=pg.mkPen((0,255,0), width=2)
                    ui.w5.plot(dataArray3,pen=ui.penGreen,clear=True)
                    #ui.w5.setXRange(dataArray3[-1][0]-400, dataArray3[-1][0])
...
def complete_skelet():
    try:
        ...
        if len(mark.data)>0:
            ...
            ui.regionlr.setBounds([0,rootX])
            ui.regionlr.setRegion([tailX-20,tailX+20])
            #TODO : remplacer cette région par une annularROI qui génère un mask annulaire par rapport à une roi
...
def tail_Track(iframe,analysimg):
    ...
    (_,tailcnts, _) = cv2.findContours(threshTailZone.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    if tailcnts :
        ...
        varM.ctailX = int(M["m10"] / M["m00"])
        varM.ctailY = int(M["m01"] / M["m00"])
        
        varM.ctailX=varM.ctailX+xroicv
        varM.ctailY=int(video.height)-varM.ctailY
        ...
        """
        ui.tailEllipse.angleList.append([iframe,tailAngleCorr])
        #ui.tailEllipse.angleList[iframe]=tailAngleCorr
        
        
    else :
        #si la queue n'est pas détectée prendre la valeur précédente
        print("tail detection failed")
        tailAngle=ui.tailEllipse.angleList[-1][1]
        ...
        #ui.tailEllipse.angleList[iframe]=tailAngle
        #solution 2 : prendre une valeur aberrante : 90 degrés
    #print("tail track ok")
    #-------------------- 
...
def eye_segmentation(roiIndex,roiEye,threshEye,analysimg):
    ...
    (_,cnts, _) = cv2.findContours(threshed_Eye.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
...
def eye_Rotation(roiIndex,roiEye,cnt):
    ...
    for contour in cnt :
        ...
        """
        #pour l'ellipse l'angle est calculé par raport à la verticale dans le sens horaire à partir de l'origine du rectangle minimum
        #angle=angle-90    
        """
        #si l'angle est positif dans le sens horaire par raport à la verticale, la transposition par rapport à l'horizontale est angle = 90-angle
        ...
        """
        #------------------------------------------------------------------------
        roisEllipseEye[roiIndex].setPos((xp,yp),update=False)
        roisEllipseEye[roiIndex].setSize((MA, ma),update=False)
        roisEllipseEye[roiIndex].setAngle(-anglep,update=False)
        #---------------------------------------------------------------------------
        """
...
def create_Result(filePath,phaseDuration):
    ...
    try :
        csvResult=csv.writer(open(filePath+".csv","w"))
...
class EllipseROI_centered_noHandle(pg.ROI):
    ...
    **Arguments**
    ...
    def __init__(self, pos, size, **args):
        ...
        self.descriptor=[0,0,0,0,0,0,0]#[xp,yp,Ma,ma,angle,vx,vy]
...
#                                                                             DYNAMIC PLOT GUI
#---------------------------------------------------------------------------------------------------------------      
class PyqtgWindow(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.win.setWindowTitle('Xenopus project - beta')
        ...
        self.d2 = Dock("Settings", size=(500,200))#(1, 1))     ## give this dock the minimum possible size
        ...
        self.label = QtgGui.QLabel(""" -- DockArea use -- 
        ...
        """)
        saveBtn = QtgGui.QPushButton('Save dock state')
        restoreBtn = QtgGui.QPushButton('Restore dock state')
        ...
        self.label8 = QtgGui.QLabel(""" Selection type  """)
        splitter = QtWidgets.QSplitter()
        splitter.setOrientation(QtCore.Qt.Vertical)
        self.selectEyes_radioButton = QtWidgets.QRadioButton(splitter)
        ...
        self.selectTailRoot_radioButton = QtWidgets.QRadioButton(splitter)
        ...
        self.selectLimbs_radioButton = QtWidgets.QRadioButton(splitter)
        self.selectLimbs_radioButton.setText("Limb")
        self.selectExclusion_radioButton = QtWidgets.QRadioButton(splitter)
        self.selectExclusion_radioButton.setText("Circle (exclusion)")
        
        completeSegmentation_btn= QtgGui.QPushButton('Complete')
        completeSegmentation_btn.clicked.connect(complete_skelet)
        initLimbTrack_btn= QtgGui.QPushButton('init limbs track')
        ...
        threshTailSlider_label.setText("Tail")
        ...
        #-----------------------------------------------------------------------------splitter_thresh
        saveConfig_btn = QtgGui.QPushButton('Save config')
        #saveConfig_btn.clicked.connect(save_Config)
        loadConfig_btn = QtgGui.QPushButton('Load config')
        #loadConfig_btn.clicked.connect(load_Config)
                
        self.w8.addWidget(self.label8, row=0, col=0)
        self.w8.addWidget(splitter, row=1, col=0,rowspan=4)
        ...
        self.videoName_label = QtgGui.QLabel(""" No video """)
        
        loadVideo_btn = QtgGui.QPushButton('Load video')
        loadVideo_btn.clicked.connect(load_video)
        
        self.connectCam_btn = QtgGui.QPushButton('Connect camera')
        ...
        self.liveVideo_btn = QtgGui.QPushButton('Live')
        ...
        self.measuredLivefps_Label=QtgGui.QLabel(""" 00 fps """)
        ...
        """        
        self.importLive_btn = QtgGui.QPushButton('import Live')
        self.importLive_btn.clicked.connect(import_live)
        self.importLive_btn.setEnabled(False)
        """
        resetPlots_btn = QtgGui.QPushButton('Reset plots')
        resetPlots_btn.clicked.connect(reset_Plot)
        resetBuffer_btn = QtgGui.QPushButton('Reset buffer')
        resetBuffer_btn.clicked.connect(reset_Buffer)
        saveResults_btn = QtgGui.QPushButton('Save results')
        saveResults_btn.clicked.connect(save_ResultsNow)
        
        self.playVideo_btn = QtgGui.QPushButton('Play/Pause')
        ...
        self.progressBuffer=QtgGui.QProgressBar()
        ...
        self.track_checkBox = QtgGui.QPushButton('Track')
        ...
        self.stepFwdVideo_btn = QtgGui.QPushButton('step >')
        self.stepFwdVideo_btn.clicked.connect(forward_Video)
        self.stepFwdVideo_btn.setEnabled(False)
        self.stepBwdVideo_btn = QtgGui.QPushButton('< step')
        ...
        numFrame_label = QtgGui.QLabel("00")
        numFrame_label.setAlignment(QtCore.Qt.AlignCenter)
        
        self.lenBuffer_label = QtgGui.QLabel("00")
        ...
        self.measuredPlayfps_Label=QtgGui.QLabel(""" 00 fps """)
...
if __name__ == '__main__':
    import sys
    #if (sys.flags.interactive != 1) or not hasattr(QtgCore, 'PYQT_VERSION'):
    #    QtgGui.QApplication.instance().exec_()
